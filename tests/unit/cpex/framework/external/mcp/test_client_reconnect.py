# -*- coding: utf-8 -*-
"""Location: ./tests/unit/cpex/framework/external/mcp/test_client_reconnect.py
Copyright 2025
SPDX-License-Identifier: Apache-2.0

Unit tests for MCP external plugin client reconnection logic.
Ported from ContextForge main (#3639).
"""
...
class TestReconnectConfiguration:
    ...
    async def test_reconnect_config_defaults(self):
        config = PluginConfig(
            ...
            kind="external",
            hooks=["tool_pre_invoke"],
            mcp=MCPClientConfig(proto=TransportType.STREAMABLEHTTP, url="http://localhost:9000/mcp"),
        )
...
class TestReconnectSession:
    ...
    async def test_reconnect_http_success_on_first_attempt(self, mock_http_plugin_config):
        plugin = ExternalPlugin(mock_http_plugin_config)
        plugin._reconnect_attempts = 3
        plugin._reconnect_delay = 0.1

        with patch.object(plugin, "_cleanup_session", new_callable=AsyncMock) as mock_cleanup:
            with patch.object(
                plugin, "_ExternalPlugin__connect_to_http_server", new_callable=AsyncMock
            ) as mock_connect:
    ...
    async def test_reconnect_stdio_success_on_first_attempt(self, mock_stdio_plugin_config):
        ...
        plugin._reconnect_delay = 0.05

        with patch.object(plugin, "_cleanup_session", new_callable=AsyncMock) as mock_cleanup:
            with patch.object(
                plugin, "_ExternalPlugin__connect_to_stdio_server", new_callable=AsyncMock
            ) as mock_connect:
                await plugin._reconnect_session()
                mock_cleanup.assert_called_once()
    ...
    async def test_reconnect_linear_backoff(self, mock_http_plugin_config):
        ...
        with patch.object(plugin, "_cleanup_session", new_callable=AsyncMock):
            with patch.object(
                plugin, "_ExternalPlugin__connect_to_http_server", new_callable=AsyncMock, side_effect=mock_connect_fail
            ):
                with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
                    await plugin._reconnect_session()
                    assert mock_sleep.call_count == 2
                    calls = mock_sleep.call_args_list
                    assert calls[0][0][0] == 0.1  # 0.1 * 1
                    assert calls[1][0][0] == 0.2  # 0.1 * 2
    ...
    async def test_reconnect_attempts_exhausted(self, mock_http_plugin_config):
        ...
        with patch.object(plugin, "_cleanup_session", new_callable=AsyncMock):
            with patch.object(
                plugin,
                "_ExternalPlugin__connect_to_http_server",
                ...
                with patch("asyncio.sleep", new_callable=AsyncMock):
                    with pytest.raises(PluginError) as exc_info:
                        await plugin._reconnect_session()
                    assert "Failed to reconnect" in str(exc_info.value.error.message)
                    assert "2 attempts" in str(exc_info.value.error.message)
...
class TestInvokeHookWithReconnection:
    ...
    @pytest.mark.asyncio
    async def test_invoke_hook_reconnect_failure_raises_original_error(
        self, mock_http_plugin_config, mock_plugin_context
    ):
        plugin = ExternalPlugin(mock_http_plugin_config)
        mock_session = AsyncMock()
        plugin._session = mock_session
        ...
        with patch("cpex.framework.external.mcp.client.get_hook_registry") as mock_registry:
            ...
            with patch.object(
                ...
                side_effect=PluginError(
                    error=PluginErrorModel(message="Reconnection failed", plugin_name="TestHTTPPlugin")
                ),
            ):
                payload = ToolPreInvokePayload(name="test", args={})
                with pytest.raises(PluginError) as exc_info:
    ...
    async def test_invoke_hook_session_terminated_reconnect_failure_reraises_original(
        ...
        async def mock_call_tool(*args, **kwargs):
            raise PluginError(
                error=PluginErrorModel(message="Session terminated by server", plugin_name="TestHTTPPlugin")
            )
