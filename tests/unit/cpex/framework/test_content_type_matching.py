...
from cpex.framework.utils import matches, normalize_content_type
...
class TestContentTypeMatching:
    ...
    def test_matches_content_type_all_conditions_combined(self):
        ...
        condition = PluginCondition(
            server_ids={"srv1"}, tenant_ids={"tenant1"}, user_patterns=["admin"], content_types=["application/json"]
        )

        # All match
        context1 = GlobalContext(
            request_id="req1", server_id="srv1", tenant_id="tenant1", user="admin_user", content_type="application/json"
        )
        assert matches(condition, context1) is True

        # One condition fails
        context2 = GlobalContext(
            request_id="req2", server_id="srv1", tenant_id="tenant1", user="admin_user", content_type="text/plain"
        )
        assert matches(condition, context2) is False
