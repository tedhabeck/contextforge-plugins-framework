...
def sample_global_context():
    ...
    """
    return GlobalContext(request_id="test-req-123", user="test_user", tenant_id="test-tenant", server_id="test-server")
...
def sample_plugin_context(sample_global_context):
    ...
    return PluginContext(
        global_context=sample_global_context, state={"test_key": "test_value"}, metadata={"test_meta": "test_data"}
    )
...
def mock_communicator():
    ...
    mock_comm.send_task = MagicMock(
        return_value={"continue_processing": True, "modified_payload": None, "violation": None, "metadata": {}}
    )
    return mock_comm
